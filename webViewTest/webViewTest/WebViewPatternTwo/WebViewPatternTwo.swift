...
public struct WebViewState: Equatable {
    ...
    public internal(set) var pageScript: String?
    public internal(set) var pageStyleSheets: String?
    ...
    public static func == (lhs: WebViewState, rhs: WebViewState) -> Bool {
        lhs.isLoading == rhs.isLoading
            ...
            && lhs.pageScript == rhs.pageScript
            && lhs.pageStyleSheets == rhs.pageStyleSheets
    ...
    public func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
      setLoading(false,
        ...
        if self.webView.htmlInState {
            ...
            webView.evaluateJavaScript("document.currentScript") { (response, error) in
                if let scripts = response as? String {
                    var newState = self.webView.state
                    newState.pageScript = scripts
                    self.webView.state = newState
                }
            }
            webView.evaluateJavaScript("document.styleSheets[0].cssRules[0].cssText.toString()") { (response, error) in
                if let styleSheets = response as? String {
                    var newState = self.webView.state
                    newState.pageStyleSheets = styleSheets
                    self.webView.state = newState
                }
            }
        }
